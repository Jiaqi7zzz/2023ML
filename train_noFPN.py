...
from nets.yolo import YoloBody,YoloBody_noFPN
...
if __name__ == "__main__":
    ...
    anchors_mask_noFPN = [[6 , 7 , 8] , [6 , 7 , 8] , [ 6 , 7 , 8]]
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #   创建yolo模型
    #------------------------------------------------------#
    model = YoloBody_noFPN(anchors_mask_noFPN, num_classes, pretrained=pretrained)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    yolo_loss = YOLOLoss(anchors, num_classes, input_shape, Cuda, anchors_mask_noFPN)
    #----------------------#
    #   记录Loss
    #----------------------#
    if local_rank == 0:
    ...
    if local_rank == 0:
        show_config(
            classes_path = classes_path, anchors_path = anchors_path, anchors_mask = anchors_mask_noFPN, model_path = model_path, input_shape = input_shape, \
    ...
    if True:
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, anchors, anchors_mask_noFPN, class_names, num_classes, val_lines, log_dir, Cuda, \
