...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = np.transpose(self.preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=0.3, hue=0.1, sat=0.7, val=0.4, random=True):
        ...
        image_data[..., 0] = cv2.LUT(hue, lut_hue)
        image_data[..., 1] = cv2.LUT(sat, lut_sat)
        image_data[..., 2] = cv2.LUT(val, lut_val)
